...
class BitField(object):
    ...
    def __init__(self, from_bit: int, to_bit: int) -> None:
        ...
        self.mask = ((1 << (to_bit - from_bit + 1)) - 1) << from_bit
    ...
    def extract(self, word: int) -> int:
        ...
        return masked_word >> self.from_bit
    ...
    def insert(self, value: int, word: int) -> int:
        ...
        value &= (1 << (self.to_bit - self.from_bit + 1)) - 1

        return (word & ~self.mask) | (value << self.from_bit)

    def extract_signed(self, word: int) -> int:
        """Extract bits in bitfield as a signed integer."""
        val = self.extract(word)
        return sign_extend(val,self.mask.bit_count())
