...
class Read(Expr):
    ...
    def gen(self, context: Context, target: str):
        ...
        self.expr.gen(context, target)
        context.add_line(f"   STORE  {target},r0,r0[510]")
...
class Comparison(Control):
    ...
    In the mpiler, "if" and "while" delegate that branching
...
class If(Control):
    ...
    def gen(self,context: Context, target: str):
        ...
        # condition check
        self.cond.condjump(context, target,else_label, jump_cond=False)  # jump to else if condition is false
        # then part
        self.thenpart.gen(context, target)
        context.add_line(f"JUMP {fi_label}")  # jump to end of if statement
        # else part
